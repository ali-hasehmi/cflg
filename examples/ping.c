...
int main(int argc, char *argv[]) {
  ...
  flgset_bool(&fset, &timestamps, 'D', NULL,
              "Print timestamp before each line.");
  flgset_bool(&fset, &precision, '3', NULL,
              "RTT precision, do not round (legacy).");
  flgset_bool(&fset, &ipv4_only, '4', NULL, "Use IPv4 only.");
  flgset_bool(&fset, &ipv6_only, '6', NULL, "Use IPv6 only.");

  // Integer Flags
  flgset_int64(&fset, &count, 'c', "", "<NUM>",
               "Stop after sending <NUM> packets.");
  flgset_int(&fset, &deadline, 'w', "deadline", "<SECS>",
             "Exit after <SECS> seconds.");
  flgset_int(&fset, &ttl, 't', "ttl", "<NUM>", "Set the IP Time to Live.");
  flgset_int(&fset, &mark, 'm', "mark", "<MARK>",
             "Set the fwmark on outgoing packets.");
  flgset_uint(&fset, &packet_size, 's', "size", "<NUM>",
              "Set the packet size in bytes.");
  flgset_uint(&fset, &ident, 0, "identifier", "<NUM>",
              "Set the identifier (process ID).");

  // Floating-Point Flags
  flgset_float(&fset, &interval, 'i', "interval", "<TIME>",
               "Wait <TIME> seconds between sending each packet.");
  flgset_double(&fset, &timeout, 'W', "timeout", "<TIME>",
                "Time to wait for a response, in seconds.");

  // String Flags
  flgset_string(&fset, &interface, 'I', "interface", "<IFACE>",
                "Send packets through this interface.");
  ...
  printf("  %-*s %s\n", label_width,
         "High Precision (legacy):", btoa(precision));
  printf("\n");
